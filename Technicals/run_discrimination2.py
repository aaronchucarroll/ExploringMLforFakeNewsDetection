...
def main(_):
    ...
    for split in splits_to_predict:
        ...
        # labels (correct annotations)
        # probs (prediction)
        print("Size of labels = %s, size of preds = %s" % (labels.size, preds.size))
        for idx in range(probs.size):
            if (idx < labels.size or idx < preds.size):
                print("%s :: %s" % (labels[idx], preds[idx]))
